    ...
    <div className="page">
      <div className="page__header">
        <div className="page__header-content">
          ...
          <p className="page__subtitle">Download compliance certificates, manage declarations, and review your blockchain carbon credit ledger.</p>
        </div>
      </div>
      ...
      <div className="page__content">
        ...
        {activeTab === 'vat204' && <Vat204FormEmbedded />}
...
/* ─── Embedded VAT 204 Form ─── */

function Vat204FormEmbedded() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [isDrawing, setIsDrawing] = useState(false);
  const [hasSigned, setHasSigned] = useState(false);
  const [submitted, setSubmitted] = useState(false);
  ...
  if (submitted) {
    return (
      <div style={{ padding: '40px', textAlign: 'center' }}>
        <FiCheckCircle size={48} style={{ color: 'var(--success)', marginBottom: '16px' }} />
        <h2 style={{ marginBottom: '8px' }}>VAT 204 Declaration Submitted</h2>
        <p style={{ color: 'var(--text-muted)', marginBottom: '24px' }}>
          Your signed VAT collection declaration has been recorded.
        </p>
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
          <button className="btn btn--primary" onClick={generateVatPdf}>
            <FiDownload size={14} style={{ marginRight: '8px' }} /> Download PDF
          </button>
          <button className="btn btn--ghost" onClick={() => { setSubmitted(false); clearSignature(); }}>
            <FiFileText size={14} style={{ marginRight: '8px' }} /> Submit Another
          </button>
        </div>
      </div>
    );
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div style={{ ...cardStyle, marginBottom: '0' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '16px' }}>
          <input type="checkbox" name="declaration" checked={formData.declaration} onChange={handleChange} id="vat204Declaration" required style={{ marginTop: '4px' }} />
          <label htmlFor="vat204Declaration" style={{ fontSize: '0.875rem', lineHeight: 1.5 }}>
