...
class AuthService {
    ...
    async signIn(email, password) {
        ...
        // ROLE-BASED ACCESS CONTROL: Check if user has admin platform access
        if (data.user) {
            try {
                ...
                if (userRole.status !== 'active') {
                    ...
                    const statusMessage = userRole.status === 'pending' 
                        ...
                        : `Your admin account is ${userRole.status}.`;
                    return { 
                        data: null, 
                        error: new Error(statusMessage + ' Please contact a system administrator.') 
                    };
                }

                // Additional check: verify admin record exists
                ...
                if (!adminData) {
                    ...
                    return { 
                        data: null, 
                        error: new Error('Admin record not found. Please contact a system administrator.') 
                    };
                }
                ...
                if (adminData.status !== 'active') {
                    ...
                    return { 
                        data: null, 
                        error: new Error(`Admin account is ${adminData.status}. Please contact a system administrator.`) 
                    };
                }
                ...
                return { 
                    data: null, 
                    error: new Error('Unable to verify admin permissions. Please try again.') 
                };
            }
        }
